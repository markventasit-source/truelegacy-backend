  ...
  try {
    ...
    if (blog) {
      ...
      const replaceMeta = (tagName, attrName, attrValue, newContent) => {
        const regex = new RegExp(`<meta[^>]*${attrName}=["']${attrValue}["'][^>]*>`, 'i');
        if (regex.test(html)) {
          html = html.replace(regex, `<meta ${attrName}="${attrValue}" content="${newContent}" />`);
        } else {
          // If tag doesn't exist, prepend it to </head>
          html = html.replace('</head>', `<meta ${attrName}="${attrValue}" content="${newContent}" />\n</head>`);
        }
      };
